...
describe("Config Handler — Full Agent Field Forwarding", () => {
  it("registers strategist as primary, rest as subagents", async () => {
    const config: any = { agent: {}, commands: [], plugin: ["opencode-ollama-orchestrator"] };
  ...
  it("forwards every user-configurable field from opencode.json agent block", async () => {
    ...
    expect(s.tools).toEqual(expect.objectContaining({ bash: true, task: true, doom_loop: true }));
  ...
  it("loads plugin-level smallModel into orchestrator defaults", async () => {
    const config: any = {
      ...
      commands: [],
      plugin: [
        ["opencode-ollama-orchestrator", { smallModel: "ollama/gemini-3-flash-preview" }],
      ],
    };
    const handler = createConfigHandler({ agents: AGENTS });
    await handler(config);
    expect(config.agent.engineer.smallModel).toBe("ollama/gemini-3-flash-preview");
  });
  ...
  it("forward plugin-level loop defaults", async () => {
    const config: any = {
      ...
      commands: [],
      plugin: [
        ["opencode-ollama-orchestrator", { defaultAllowLoop: true, defaultLoopCount: 4 }],
      ],
    };
    const handler = createConfigHandler({ agents: AGENTS });
    await handler(config);
