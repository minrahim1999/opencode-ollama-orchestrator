...
export class MissionController {
	...
	private setupShutdownHandlers(): void {
		const shutdown = async (signal: string) => {
			...
			if (this.runningTasks.size > 0) {
				...
				console.error(
					`[opencode-orchestrator] Waiting for ${tasks.length} running tasks...`,
				);
				try {
					await Promise.race([
						Promise.all(tasks),
					...
					console.error(
						"[opencode-orchestrator] Task drain timed out, saving state and exiting",
					);
				}
			}
	...
	async start(description: string, auto = false): Promise<void> {
		...
		const _dirs = ensureProjectDirs(this.deps.directory);
		...
		if (cfg.doxAutoInit) {
			...
			if (!doxCheckResult.ok) {
				this.emit(
					ctx,
					`DOX check: missing ${doxCheckResult.missing.join(", ")}`,
					auto,
				);
			}
		...
		const archSession = await this.createSession(
			names.architect,
			`Plan: ${slug}`,
			undefined,
			slug,
		...
		if ((ctx.state as MissionState) !== "failed") {
			...
			this.emit(
				ctx,
				`✅ MISSION_COMPLETE ${ctx.completedAt - parseMissionTimestamp(ctx.missionId)}ms`,
				auto,
			);
		}
	...
	async resume(): Promise<void> {
		...
		for (const ctx of active) {
			...
			if (ctx.state !== "failed") {
				...
				this.emit(
					ctx,
					`✅ MISSION_RESUMED_COMPLETE ${ctx.completedAt - parseMissionTimestamp(ctx.missionId)}ms`,
					true,
				);
			}
	...
	abortMission(slug: string): boolean {
		for (const [, ctx] of Array.from(this.missions.entries())) {
			if (ctx.slug === slug) {
				...
				Logger.log("warn", "mission-controller", `Mission ${slug} aborted`, {
					mission: slug,
				});
				return true;
			}
	...
	resumeFrom(missionSlug: string, taskId: string): boolean {
		for (const [, ctx] of Array.from(this.missions.entries())) {
			if (ctx.slug === missionSlug) {
				...
				if (!found) {
					console.error(
						`[opencode-orchestrator] Task ${taskId} not found in mission ${missionSlug}`,
					);
					return false;
				}
				...
				this.executeTodos(ctx, cfg, names, true).catch((err) => {
					console.error(
						`[opencode-orchestrator] resumeFrom execute error:`,
						err,
					);
				});
				return true;
	...
	async checkWatchdog(): Promise<void> {
		...
		for (const [sid, sess] of Array.from(this.deps.sessions.entries())) {
			...
			if (idle > STUCK_THRESHOLD_MS) {
				Logger.log(
					"warn",
					"watchdog",
					`Killing stuck session ${sid.slice(0, 8)}`,
					{
						agent: sess.agent,
		...
		if (killed.length > 0) {
			Logger.log(
				"info",
				"watchdog",
				`Killed ${killed.length} stuck session(s)`,
				{ count: killed.length },
			);
		}
	}
	...
	private async sendNotification(
		event: Parameters<typeof notify>[1],
	): Promise<void> {
		try {
	...
	revertMission(slug: string): boolean {
		for (const [, ctx] of Array.from(this.missions.entries())) {
			if (ctx.slug === slug) {
				if (!ctx.backup || ctx.backup.type === "none") {
					console.error(
						`[opencode-orchestrator] Mission ${slug} has no backup. Nothing to revert.`,
					);
					return false;
				}
				ctx.state = "failed";
				...
				if (ok) {
					this.emit(
						ctx,
						`↩️ Mission ${slug} reverted to pre-mission state`,
						true,
					);
					...
					this.emit(
						ctx,
						`❌ Mission ${slug} revert FAILED — manual intervention required`,
						true,
					);
				}
				return ok;
		...
		Logger.log(
			"warn",
			"mission-controller",
			`Mission ${slug} not found for revert`,
		);
		return false;
	}
	...
	status(): string {
		...
		for (const ctx of Array.from(this.missions.values())) {
			...
			const inProgress = ctx.todos.filter(
	...
	private async executeTodos(
		ctx: MissionCtx,
		cfg: ReturnType<typeof loadOrchestratorConfig>,
		names: ReturnType<typeof loadOrchestratorConfig>["names"],
		auto: boolean,
	) {
		// Track this mission as running (for graceful shutdown)
		const executePromise = this._executeTodosInner(
			ctx,
			cfg,
			names,
			auto,
	...
	private async _executeTodosInner(
		ctx: MissionCtx,
		cfg: ReturnType<typeof loadOrchestratorConfig>,
		names: ReturnType<typeof loadOrchestratorConfig>["names"],
		auto: boolean,
		...
		while (true) {
			...
			if (currentPhase && isPhaseComplete(currentPhase)) {
				const gateTask = ctx.todos.find(
					(t) => t.phase === currentPhase && t.phaseGate,
				);
			...
			while (running.size < maxWorkers && !phaseGatePending) {
				...
				if (todo.phase && todo.phase !== currentPhase) {
					...
					if (currentPhase) {
						const prevGate = ctx.todos.find(
							(t) => t.phase === currentPhase && t.phaseGate,
						);
						if (prevGate && prevGate.status !== "completed") {
							this.emit(
								ctx,
								`${todo.id} deferred: phase "${currentPhase}" gate not complete`,
								auto,
							);
							break;
						}
				...
				this.emit(
					ctx,
					`Delegating ${todo.id} to ${resolveAgentAlias(todo.agent, names)}`,
					auto,
				);
		...
		if (allDone) {
			...
			const doneCount = ctx.todos.filter(
	...
	private async runTask(
		...
		try {
			const session = await this.createSession(
				resolvedAgent,
				`${todo.id}: ${todo.description.slice(0, 40)}`,
				todo.id,
				ctx.slug,
			);
			...
			if (todo.criticalPath) {
				...
				if (!auditPassed) {
					...
					if (retries < maxRetries) {
						...
						this.emit(
							ctx,
							`${todo.id} audit failed, retry ${retries + 1}/${maxRetries}`,
							auto,
						);
					...
					updateTodoStatus(
						...
						todo.id,
						"failed",
						`Audit failed after ${maxRetries} retries`,
						ctx.slug,
					);
			...
			if (retries < maxRetries) {
				...
				this.emit(
					ctx,
					`${todo.id} failed (${String(err).slice(0, 100)}), retry ${retries + 1}/${maxRetries}`,
					auto,
				);
				await sleep(1000 * 2 ** retries);
				return this.runTask(todo, index, ctx, cfg, names, auto);
	...
	private buildMemoryContext(ctx: MissionCtx): string | undefined {
		...
		for (const m of entries) {
			...
			if (m.filesChanged.length)
				lines.push(`  - Files: ${m.filesChanged.join(", ")}`);
			if (m.issues.length) lines.push(`  - Issues: ${m.issues.join("; ")}`);
		}
		lines.push(
			"",
			"Use this context to avoid duplicating work or re-introducing already-fixed issues.",
		);
		return lines.join("\n");
	}

	/** Add a task memory entry */
	private addTaskMemory(
		ctx: MissionCtx,
		taskId: string,
		agent: string,
		summary: string,
	...
	private async runAudit(
		...
		try {
			if (existsSync(auditResultPath)) {
				...
				this.emit(
					ctx,
					`❌ AUDIT ${todo.id}: FAILED — ${result.issues?.join("; ") || "No details"}`,
					true,
				);
				return false;
			}
		...
		try {
			...
			if (client?.v2?.session?.messages) {
				...
				if (data) {
					...
					const textParts = lastMsg?.parts as
						| Array<{ text?: string }>
						| undefined;
					const text =
						(lastMsg?.info?.text as string | undefined) ||
		...
		this.emit(
			ctx,
			`⚠️ AUDIT ${todo.id}: No explicit result found, defaulting to PASS`,
			true,
		);
		return true;
	}
	...
	private buildTaskPrompt(
		todo: ParsedTodo,
		cfg: Pick<
			ReturnType<typeof loadOrchestratorConfig>,
			"maxRetries" | "requireApproval" | "maxSubagentDepth"
		>,
	...
	private emit(ctx: MissionCtx, message: string, auto: boolean) {
		...
		Logger.log("info", "mission-controller", message, {
			mission: ctx.slug,
			mode,
			state: ctx.state,
		});
				...
				client.tui
					.showToast({
						body: {
							...
							variant,
							duration:
								variant === "error" || variant === "warning" ? 8000 : 5000,
						},
					})
					.catch(() => {
