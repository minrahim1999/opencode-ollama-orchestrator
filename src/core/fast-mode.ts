...
export class FastModeController {
  ...
  startWatch(watchIntervalMs = 5000): void {
    ...
    this.isRunning = true;
    Logger.log("info", "fast-mode", "Watcher started (interval=" + watchIntervalMs + "ms)");
  ...
  async tick(): Promise<void> {
    ...
    if (!mission || mission.state !== "queued") return;
    ...
    if (exhausted?.exhausted) {
      Logger.log("error", "fast-mode", `Token budget exhausted. Pausing mission ${next}.`);
    ...
    const timeoutHandle = setTimeout(() => {
      Logger.log("warn", "fast-mode", `Mission ${next} timed out after ${timeoutMs}ms`);
  ...
  validateResponse(response: string, scopeFiles: string[]): GuardResult {
    if (!this.guardFn) {
      return { approved: true, confidence: 1, violations: [], recommendation: "proceed" };
    }
    const result = this.guardFn(response, scopeFiles);

    const mission = this.activeMission ? this.missions.get(this.activeMission) : undefined;
  ...
  status(): string[] {
    ...
    for (const m of allMissions) {
      const elapsed = m.state === "completed" && m.completedAt
        ? `${Math.round((m.completedAt - m.createdAt) / 1000)}s`
        : `${Math.round((Date.now() - m.createdAt) / 1000)}s`;
      lines.push(`  ${m.slug}: ${m.state} (${elapsed}) — violations=${m.guardViolations}`);
    }
    return lines;
  }
  ...
  resume(slug: string): boolean {
    ...
    if (!mission || mission.state !== "paused") return false;
