...
export class TokenBudgetManager {
  ...
  startTask(taskId: string): BudgetSnapshot {
    ...
    Logger.log("debug", "token-budget", `Task ${taskId} started (budget=${this.config.maxTokensPerTask})`);
    return this.getSnapshot(taskId);
  }
  ...
  consume(taskId: string, estimatedTokens: number): BudgetSnapshot {
    ...
    taskBudget.remaining = Math.max(0, this.config.maxTokensPerTask - taskBudget.consumed);
    this.state.totalConsumed += estimatedTokens;
    this.state.contextWindowUsed += estimatedTokens;

    const snap = this.getSnapshot(taskId);

    Logger.log("debug", "token-budget", `Task ${taskId} consumed ${estimatedTokens} (remaining=${snap.contextWindowRemaining})`);
  ...
  shouldSummarize(): boolean {
    ...
    const ratio = this.state.contextWindowUsed / this.config.contextWindowBudget;
    return ratio >= 0.8;
  }
  ...
  summarize(): { clearedWindow: number; summarizeCount: number } {
    ...
    Logger.log("info", "token-budget", `Context summarized. Cleared ${cleared} tokens (total consumed=${this.state.totalConsumed})`);

    return { clearedWindow: cleared, summarizeCount: this.state.summarizeCount };
  }

  /** Hard kill check — total mission budget exhausted */
  isExhausted(): { exhausted: boolean; totalConsumed: number; budget: number } {
    const exhausted = this.state.totalConsumed >= this.config.contextWindowBudget;
    return {
      exhausted,
  ...
  static condenseMemory(entries: Array<{ taskId: string; summary: string; filesChanged: string[]; issues: string[] }>): string {
    const parts: string[] = [];
    for (const e of entries.slice(-3)) {
      const files = e.filesChanged.length > 0 ? `(${e.filesChanged.join(", ")})` : "(no file changes)";
      const issues = e.issues.length > 0 ? `[issues: ${e.issues.join(", ")}]` : "";
  ...
  private getSnapshot(taskId: string): BudgetSnapshot {
    ...
    const remaining = Math.max(0, this.config.contextWindowBudget - this.state.contextWindowUsed);
    const allowed = consumed < this.config.maxTokensPerTask && this.state.contextWindowUsed < this.config.contextWindowBudget;
    ...
    return { totalConsumed: this.state.totalConsumed, contextWindowRemaining: remaining, taskId, allowed, reason };
  }
}
