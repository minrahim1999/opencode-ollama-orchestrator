...
export async function notify(
  config: NotifyConfig,
  event: {
    type: "mission_started" | "mission_completed" | "mission_failed" | "mission_stuck" | "task_failed" | "backup_created";
    missionSlug: string;
    message: string;
  ...
  try {
    const resp = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        priority: event.type === "mission_failed" || event.type === "mission_stuck" ? 5 : 3,
...
async function sendWebhook(url: string, headers: Record<string, string>, event: any): Promise<void> {
  try {
    const resp = await fetch(url, {
      method: "POST",
