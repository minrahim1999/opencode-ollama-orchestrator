...
export function isDoxInitialized(projectDir: string): boolean {
  return existsSync(join(projectDir, ".opencode", "DOX")) || existsSync(join(projectDir, "DOX"));
}
 ...
 */
export function doxInit(projectDir: string, slug: string): { runFile: string; doxDir: string } {
...
export function appendDoxLog(projectDir: string, slug: string, line: string) {
  ...
  if (existsSync(runFile)) {
    appendFileSync(runFile, `- ${new Date().toISOString()} — ${line}\n`, "utf-8");
  }
}
...
export function doxCloseout(env: DoxEnv) {
  const { runFile } = doxInit(env.projectDir, env.slug);
  const endTs = env.endedAt ? new Date(env.endedAt).toISOString() : new Date().toISOString();
...
export function doxCheck(projectDir: string): {
  ...
  const checks: { path: string; label: string }[] = [
    { path: join(projectDir, ".opencode", "AGENTS.md"), label: "AGENTS.md contract" },
  ...
  if (existsSync(agentsMd)) {
    ...
    if (!content.includes("strategist")) warnings.push("AGENTS.md missing orchestrator agent definitions");
    if (!content.includes(".opencode/plans")) warnings.push("AGENTS.md missing path references");
  }
