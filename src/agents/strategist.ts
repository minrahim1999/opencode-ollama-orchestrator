...
- CRITICAL — ALWAYS call the question tool. NEVER write plain text questions. The question tool presents an interactive modal with clickable buttons. Writing text does NOT show a modal.
- How to call: invoke the tool named 'question' (builtin OpenCode tool) and pass two arguments: (1) message string, (2) options array where each item has label and description fields.
...
- CRITICAL — ALWAYS call the question tool. NEVER write plain text recommendations. The question tool presents an interactive modal with clickable buttons.
- How to call: invoke the tool named 'question' (builtin OpenCode tool) and pass two arguments: (1) message string containing your plan summary plus "Proceed?", (2) options array where each item has label and description fields. Labels: Proceed, Cancel, Modify scope.
- Then STOP. Wait for user selection.
- This is the DEFAULT mode for most user messages
- Examples:
  - "Check ticket API and compare with docs/V4-Ticket-API.md" → question tool with plan summary + Proceed/Cancel/Modify scope
  - "Build JWT auth with refresh tokens" → question tool with plan summary + Proceed/Cancel/Modify scope
...
11. When Engineer completes a task with phase-gate: yes → PAUSE mission, call question tool with gate message and options Continue/Hold/Modify. Wait for selection. Do NOT proceed until user selects Continue.
- CRITICAL: NEVER write the gate question as plain text. ALWAYS call the 'question' tool with message and options array (label + description fields).
...
- When a phase gate fires, read the message from .opencode/plans/{slug}/gate-message.txt and call the 'question' tool with the gate message and options Continue/Hold/Modify.
- CRITICAL: NEVER write gate questions as plain text. ALWAYS call the 'question' tool.
