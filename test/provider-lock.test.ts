...
describe("provider-lock (no-op mode)", () => {
	...
	function writeOpencodeJson(
		content: any,
		pathParts: string[] = [".config", "opencode"],
	) {
	...
	it("logs default model when set", async () => {
		...
		expect(spy).toHaveBeenCalledWith(
			expect.stringContaining("Provider lock DISABLED"),
		);
		expect(spy).toHaveBeenCalledWith(
			expect.stringContaining("default: ollama/deepseek-v4-pro"),
		);
		spy.mockRestore();
	});
	...
	it("logs '(unset)' when no model configured", async () => {
		...
		await lockProviderToOllama(null);
		expect(spy).toHaveBeenCalledWith(
			expect.stringContaining("default: (unset"),
		);
		spy.mockRestore();
	});
	...
	it("logs per-agent models", async () => {
		...
		await lockProviderToOllama(null);
		expect(spy).toHaveBeenCalledWith(
			expect.stringContaining("strategist: a/b"),
		);
